...
ScrollTrigger.refresh(true)
$('html').removeClass('loading');},onComplete:()=>{transitions.removeClass('half')}});innerTl.play()}},100)}else{window.onload=function(){setTimeout(function(){if(overlays.hasClass('columns')){let div=$('.page-loader > div').not('.page-loader-overlays');innerTl.to(div,{opacity:0,yPercent:-100,ease:'power3.in'})
innerTl.to('.loader-col',{height:'0%',delay:0,id:'loader-end',stagger:{grid:[1,20],from:"random",amount:.3,},duration:1.2,ease:'expo.out',onStart:()=>{let instance=ScrollTrigger.getAll(),i=0;for(i=0;i<instance.length;i++){instance[i].enable();}
ScrollTrigger.refresh(true)
$('html').removeClass('loading');},onComplete:()=>{transitions.removeClass('half')}})
innerTl.play()}else if(overlays.hasClass('overlay')){let div=$('.page-loader > div').not('.page-loader-overlays');innerTl.to(div,{opacity:0,yPercent:-100,ease:'power3.in'})
...
tl.to(capt,{width:wido,duration:1,ease:'expo.inOut'},'label_'+snapPerc);}}).done(function(){});}};var naylaSmoother=false;function smoothScroll(){let body=$('body');if(body.hasClass('smooth-scroll')){naylaSmoother=true;$('#page').wrap('<div id="smooth-wrapper"></div>');$('main').wrap('<div id="smooth-content"></div>');if((!siteHeader.hasClass('fixed'))&&(!siteHeader.hasClass('sticky'))){siteHeader.prependTo('#smooth-content')
...
return()=>{}});})}
function showcase3dTitles(){let showcase3dTitles=$('.showcase-3d-titles');showcase3dTitles.each(function(){let $this=$(this),projectsWrap=$this.find('.s3t-projects-wrap'),project=$this.find('.showcase-project'),imagesWrap=$this.find('.s3t-images'),fontSize=parseInt($this.find('.project-title').css('font-size'));$this.on('mousemove',function(e){let xTo=gsap.quickTo(imagesWrap,"left",{duration:0.6,ease:"power3"}),yTo=gsap.quickTo(imagesWrap,"top",{duration:0.6,ease:"power3"});function icko(e){xTo(e.pageX);yTo(e.pageY-$this.offset().top);}
...
let count=0;$this.find('.project-title').on('mouseenter',()=>{imagesWrap.addClass('active');if($this.hasClass('active')){imagesWrap.addClass('active');let activeImage=$('.image_'+i).parent('.d-img');activeImage.addClass('active')
...
return()=>{$('.project-title span').on('mousemove')}});const lenis=new Lenis({infinite:true,smooth:true,infinite:true,smoothTouch:false,duration:2});window.addEventListener('scroll',function(){gsap.set(projectsWrap,{rotateX:this.scrollY/10000*360})
